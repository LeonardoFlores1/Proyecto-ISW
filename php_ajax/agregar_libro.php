...
<?php
	...
	if ($_FILES["archivo_01"]["error"]>0 || $_FILES["archivo_02"]["error"]>0 || $_FILES["archivo_03"]["error"]>0) {
		...
		if ($c_extencion) {
			if ($c_tamanio) {
				$sql='INSERT INTO Libro(id_usuario, nombre, edicion, volumen, precio, autor, se_intercambia, id_estado, observaciones) 
					VALUES('.$ID_Usuario.',"'.$_POST["nombre_libro"].'","'.$_POST["no_edicion"].'", "'.$_POST["no_volumen"].'",'.$_POST["precio"].',"'.$_POST["nombre_autor"].'",'.$_POST["intercambio"].', 1, "'.$_POST["observaciones"].'")';	
